...
class RAS_Tello_ARUCO_GATE(Node):
    ...
    def image_callback(self, msg):
        ...
        # align X and Y coordinates of the ArUco marker with the center of the frame 
        if cY_center < (cY_frame - 10):
            self.tello_move_up()
        elif cY_center > (cY_frame + 10):
            self.tello_move_down()
        else:
            self.tello_vertical_stop()

        if cX_center < (cX_frame - 10):
            self.tello_rotate_right()
        elif cX_center > (cX_frame + 10):
            self.tello_rotate_left()
        else:
            self.tello_horizontal_stop()
        
        if cX_center in range(cX_frame - 10, cX_frame + 10) and cY_center in range(cY_frame - 10, cY_frame + 10):
            while True:
                self.tello_move_forward()
    ...
    def tello_move_left_top(self):
        msg = Twist()
        msg.linear.y = 0.1
        msg.linear.z = 0.1
        self.publisher_.publish(msg)
        print("[INFO] Tello move left top")
    
    def tello_move_left_bottom(self):
        msg = Twist()
        msg.linear.y = 0.1
        msg.linear.z = -0.1
        self.publisher_.publish(msg)
        print("[INFO] Tello move left bottom")
    
    def tello_move_right_top(self):
        msg = Twist()
        msg.linear.y = -0.1
        msg.linear.z = 0.1
        self.publisher_.publish(msg)
        print("[INFO] Tello move right top")
    
    def tello_move_right_bottom(self):
        msg = Twist()
        msg.linear.y = -0.1
        msg.linear.z = -0.1
        self.publisher_.publish(msg)
        print("[INFO] Tello move right bottom")
    
    def tello_move_up_rotate_left(self):
        msg = Twist()
        msg.linear.z = 0.1
        msg.angular.z = 0.1
        self.publisher_.publish(msg)
        print("[INFO] Tello move up and rotate left")
